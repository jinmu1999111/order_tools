...
def api_kitchen_orders():
    ...
    for o in orders_query: orders_by_session[o.session_id].append(o)
    output = []
    for session_id, orders in orders_by_session.items():
        if not orders: continue
        item_counts = defaultdict(int)
        for item in orders: item_counts[item.item_name] += 1
        output.append({
...
def api_dashboard_sales():
    ...
    if period == 'monthly':
        ...
        sales_data = db.session.query(func.date_trunc('month', Order.timestamp), func.sum(Order.item_price)).filter(Order.timestamp >= start_date).group_by(func.date_trunc('month', Order.timestamp)).order_by(func.date_trunc('month', Order.timestamp)).all()
        labels = [d.strftime("%Y-%m") for d, _ in sales_data]
        sales = [s for _, s in sales_data]
    else:
        start_date = today - datetime.timedelta(days=29)
        sales_data = db.session.query(func.cast(Order.timestamp, db.Date), func.sum(Order.item_price)).filter(func.cast(Order.timestamp, db.Date) >= start_date).group_by(func.cast(Order.timestamp, db.Date)).order_by(func.cast(Order.timestamp, db.Date)).all()
...
def api_kitchen_status():
    is_cooking = db.session.query(Order.query.filter_by(status='pending').exists()).scalar()
