...
def qr_auth(token):
    ...
    if not table or table.qr_token_expiry < datetime.datetime.now(JST):
...
def api_add_table():
    ...
    if name and not Table.query.filter_by(name=name).first():
        ...
        db.session.commit()
        return jsonify(success=True, table={'id': new_table.id, 'name': new_table.name, 'status': new_table.status})
